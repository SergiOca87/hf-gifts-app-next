...
function UserGift({ gift, user, selectedGifts, addOrRemoveGiftFromSelection, inSelection }) {
    const { title, featured_image } = gift

    console.log(gift);
    return (
        <Card className="overflow-hidden max-w-[350px] shadow-xl group relative z-0 border-none">
            <CardHeader className="p-0 relative h-[300px] overflow-hidden" >
                ...
                <Checkbox checked={selectedGifts.includes(gift)} onClick={() => addOrRemoveGiftFromSelection(gift)} className="z-20 absolute right-3 top-2 w-8 h-8 block" />
            </CardHeader>
